...
export default function LinkScreen({ navigation }) {
  ...
  const handleAddLink = () => {
    if (!newName.trim() || !newUrl.trim()) {
      Alert.alert("Missing Information", "Please enter both a name and a link URL.");
      return;
    }
  ...
  const getFilteredLinks = () => {
    ...
    return links.filter(link => {
      ...
      if (activeFilter === 'Older') {
        return linkDate < yesterday && linkDate.toDateString() !== yesterday.toDateString();
      }
      return true;
    });
  };
  ...
  const renderLinkItem = ({ item }) => (
    ...
    >
      ...
      <View className="flex-1">
        ...
        <Text className="text-gray-500 text-xs" numberOfLines={1}>{item.url}</Text>
    ...
    <View className="flex-1" style={{ backgroundColor: '#F3E8FF' }}>
      ...
      <View className="pt-2 pb-4 px-6 bg-white rounded-b-[30px] shadow-sm z-10">
        <View className="flex-row items-center justify-between mb-4">
          <View>
            <Text className="text-gray-500 text-xs font-bold tracking-widest uppercase">My Dashboard</Text>
            <Text className="text-2xl font-extrabold text-gray-900">Saved Links ({links.length})</Text>
          </View>
          <TouchableOpacity 
      ...
      {displayedLinks.length === 0 ? (
        <View className="flex-1 items-center justify-center opacity-60 pb-20">
          ...
          <Text className="text-xl font-bold text-purple-900">
            {activeFilter === 'All' ? 'No links yet' : `No links for "${activeFilter}"`}
          </Text>
          {activeFilter === 'All' && (
      ...
      >
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} className="flex-1 justify-end">
          <TouchableOpacity className="absolute inset-0 bg-black/50" activeOpacity={1} onPress={() => setAddModalVisible(false)} />
          <View className="bg-white rounded-t-[30px] p-8 shadow-2xl">
            <View className="w-12 h-1 bg-gray-300 rounded-full self-center mb-8" />
            <Text className="text-2xl font-bold text-gray-900 mb-6">Add New Link ✨</Text>
            ...
            <View className="mb-8">
              <Text className="text-gray-500 font-bold text-xs uppercase mb-2 ml-1">Link URL</Text>
              <TextInput
                className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-4 text-gray-900 text-base"
            ...
            <View className="flex-row gap-4">
              <TouchableOpacity className="flex-1 py-4 rounded-full items-center bg-gray-100" onPress={() => setAddModalVisible(false)}>
                <Text className="text-gray-900 font-bold">Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity className="flex-1 py-4 rounded-full items-center bg-purple-600 shadow-lg shadow-purple-200" onPress={handleAddLink}>
        ...
        <View className="flex-1 justify-center items-center bg-black/30">
          ...
          <View className="bg-white w-64 rounded-2xl shadow-2xl overflow-hidden p-2">
            ...
            >
              ...
              <Text className="text-gray-800 font-semibold text-base">Copy Link</Text>
            </TouchableOpacity>
            ...
            >
              ...
              <Text className="text-red-600 font-semibold text-base">Delete Link</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
