...
export const AuthProvider = ({ children }) => {
      ...
      try {
        ...
        const subscriber = auth().onAuthStateChanged((userState) => {
          try {
            ...
            console.log('[AuthContext] Auth state changed:', userState ? 'User logged in' : 'User logged out');
          } catch (stateError) {
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      if (!idToken) {
        const error = new Error("No ID token received. Ensure Web Client ID is configured.");
