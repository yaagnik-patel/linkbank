...
class ErrorBoundary extends Component {
  ...
  componentDidCatch(error, errorInfo) {
    ...
    if (isCrashState) {
      console.log(
        "[App Error Boundary] Crash state detected, attempting recovery"
      );
  ...
  attemptRecovery = async () => {
    ...
    if (!crashDetector) {
      console.log(
        "[App Error Boundary] Crash detection not available, basic retry"
      );
    ...
    if (recovered) {
      ...
      console.log(
        "[App Error Boundary] Recovery failed, showing recovery options"
      );
    }
      ...
      return (
        <View style={styles.errorContainer}>
          <View style={styles.errorContent}>
            <Text
              style={[styles.errorTitle, isCrashState && styles.crashTitle]}
            >
              {isCrashState
                ? "App Needs Recovery"
            ...
            {isCrashState && (
              <View style={styles.recoveryInfo}>
                ...
                {Platform.OS !== "web" && __DEV__ && (
                  <TouchableOpacity
                    style={styles.refreshButton}
                    onPress={() => require("react-native").DevSettings.reload()}
                  >
                    <Text style={styles.refreshButtonText}>Reload (dev)</Text>
                  </TouchableOpacity>
                )}
              </View>
            )}
